...
class ModelArguments:
    ...
    ckp: Optional[str] = field(default="/root/autodl-tmp/Uni_Results/QA_no_pretrain_no_aug_test_epoch100_choise/VQA_RAD/checkpoint-6128")#
...
@dataclass
class DataArguments:
    img_dir: str = field(default='/root/autodl-tmp/VQA_RAD/images', metadata={"help": "Path to the training data."})
    Test_csv_path: str = field(default='/root/autodl-tmp/VQA_RAD/test_close.csv', metadata={"help": "Path to the training data."})
...
def main():
    ...
    ###################################################33
    # 在这里，假设您已经加载了状态字典到一个变量中，例如：
    # state_dict = torch.load(ckp, map_location='cpu')

    # # 然后修正状态字典中的键名
    # for key in list(state_dict.keys()):
    #     new_key = key
    #     if "llamacasual.base_model.model.model.layers" in key:
    ...
    test_name ="/root/autodl-tmp/VQA_RAD_Results/choise-checkpoint-6128/"
    closed_save_path='result_final_greedy_VQA_RAD_close'+model_args.ckp.split('/')[-3]+'_'+ model_args.ckp.split('/')[-2]+'.csv'
    ...
    open_save_path = 'result_final_greedy_VQA_RAD_open' + model_args.ckp.split('/')[-3] + '_' + model_args.ckp.split('/')[-2] + '.csv'
    with open(open_save_path, mode='w') as outfile:
            writer = csv.writer(outfile)
            writer.writerow(['Figure_path','Question','Pred','Label'])
