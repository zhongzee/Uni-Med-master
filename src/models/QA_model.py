...
class QA_model(nn.Module):
    ...
    def process_image_features(self, images):
        B = images.shape[0]
        # images encoding
    ...
    def visual_cutmix(self, input_ids_list, images_list, samples_list, tokenizer, labels=None):
        input_ids_1, input_ids_2 = input_ids_list
        images_1, images_2 = images_list
        sample_1, sample_2 = samples_list

        # 使用 process_image_features 方法处理两张图像并获取图像特征
        processed_features_1 = self.process_image_features(images_1)
        processed_features_2 = self.process_image_features(images_2)

        # 提取两组文本特征
        text_features_1 = self.llamacasual.get_input_embeddings()(input_ids_1)
        text_features_2 = self.llamacasual.get_input_embeddings()(input_ids_2)

        # 计算相似度并选择 top patches 可能需要调整，因为我们已经获取了处理后的图像特征
        # 以下为示例性修改，具体实现依赖于compute_similarity和select_top_patches的实现细节
        similarity_scores_1 = self.compute_similarity(processed_features_1, text_features_1)
        similarity_scores_2 = self.compute_similarity(processed_features_2, text_features_2)

        top_patches_1 = self.select_top_patches(processed_features_1, similarity_scores_1)
        top_patches_2 = self.select_top_patches(processed_features_2, similarity_scores_2)

        # 执行 Cut-Mix 操作
        # 注意：perform_cut_mix 的实现需要适应接收处理过的特征
        mixed_features = self.perform_cut_mix(processed_features_1, top_patches_2)

        # 结合特征并生成最终输出
        input_embedding = torch.cat([mixed_features, text_features_1], dim=1)
    ...
    def compute_similarity(self, patch_features, text_features):
        # 计算余弦相似度
        similarity = F.cosine_similarity(patch_features.unsqueeze(1), text_features.unsqueeze(0), dim=-1)
        return similarity

    def perform_cut_mix(self, patch_features, similarity_scores, top_k=5):
        # 选择每个文本特征对应的top-k个最相似的图像补丁
        top_patches = similarity_scores.topk(top_k, dim=0).indices

        # Cut-Mix操作
        mixed_features = torch.zeros_like(patch_features)
        for i in range(top_patches.size(1)):
            indices = top_patches[:, i]
            mixed_features[indices, i, :] = patch_features[indices, i, :]

        return mixed_features

    # def visual_cutmix(self, input_ids, images, sample, tokenizer, labels=None):
    #     task_related_visual_text = sample["task_related_visual_text"]
    #     task_related_text = sample["task_related_text"]
    #
    #     # 提取图像补丁特征
    #     patch_features = self.image_encoder(images)  # 假设image_encoder方法返回每个补丁的特征
    #
    #     # 提取文本特征
    #     text_features = self.llamacasual.get_input_embeddings()(input_ids)  # 假设这样可以获取文本特征
    #
    #     # 计算图像补丁特征与文本特征之间的相似度
    #     similarity_scores = self.compute_similarity(patch_features, text_features)
    #
    #     # 执行Cut-Mix操作
    #     mixed_features = self.perform_cut_mix(patch_features, similarity_scores)
    #
    #     # 结合特征并生成最终输出
    #     input_embedding = torch.cat([mixed_features, text_features], dim=1)
    #     output = self.llamacasual(inputs_embeds=input_embedding, labels=labels)
    #
    #     return output
    # def visual_cutmix(self, input_ids, images, sample,tokenizer,labels=None):
    ...
    #     task_related_visual_text = sample["task_related_visual_text"]
    #     task_related_text = sample["task_related_text"]
    #     task_related_visual_text_input_id = tokenizer(sample['input_ids'],return_tensors="pt").to('cuda')
    #
    #     B = images.shape[0]
    #     ### images encoding ###
    #     x = self.image_encoder(images) # PMC_CLIP
    #     features = x.transpose(0, 1)  # patch_num b dim
    #
    #     ### Q-Former ###
    #     query_embed = self.query_embed.weight.unsqueeze(1).repeat(1, B, 1)  # query_number, batch, dim
